...
var Influx = require('influx'),
    ...
    constants = {
        ...
        CONFIG_STARTTIME_MEASUREMENT_NAME: 'startTimeMeasurementName',
        ...
        DEFAULT_STARTTIME_MEASUREMENT_NAME: 'startTimes',
    ...
    impl = {
        ...
        validateConfig: function (scriptConfig) {
            ...
            if (!scriptConfig[constants.CONFIG_STARTTIME_MEASUREMENT_NAME]) {
                scriptConfig[constants.CONFIG_STARTTIME_MEASUREMENT_NAME] = constants.DEFAULT_STARTTIME_MEASUREMENT_NAME;
            }
        ...
        reportResults: function (instance, influxReporter, testReport) {
            ...
            while (index < testReport.latencies.length) {
                sample = testReport.latencies[index++];
                ...
                const [timestamp, runId, latency, statusCode, metricsContext] = sample;
                ...
                if (JSON.parse(JSON.stringify(instance.config[constants.CONFIG_INFLUX].matches)) === constants.CONFIG_SHOW_MATCHES) {
                    points.push([{
                        ...
                        time: timestamp,
                        value: latency / 1000000
                    }, Object.assign({
                        response: statusCode
                    }, instance.config[constants.CONFIG_STATIC_TAGS], impl.sanitizeMetricsContext(metricsContext))]);
                } else {
                    points.push([{
                        time: timestamp,
                        value: latency / 1000000
        ...
        sanitizeMetricsContext: function (metricsContext) {
            const sanitizedContext = {};

            for (const key in metricsContext) {
                if (!['string', 'number', 'boolean'].includes(typeof (metricsContext[key]))) {
                    console.log(`Metrics Context: ${key} is not a valid context.`);
                    continue;
                }

                sanitizedContext[key] = metricsContext[key];
            }

            return sanitizedContext;
        },
        reportCustomMetrics: function (instance, influxReporter, testReport) {
            const metricsCallback = (points) => function (err) {
                if (err) {
                    impl.handleError(err.message);
                } else {
                    console.log(util.format(messages.metricsReportedToInflux, points.length));
            ...
            if (testReport.histograms) {
                for (const metricName in testReport.histograms) {
                    const points = [];
                    const histogram = testReport.histograms[metricName];

                    for (const [timestamp, delta, metricsContext] of histogram) {
                        points.push([{
                            time: timestamp,
                            value: delta / 1000000
                        }, Object.assign(instance.config[constants.CONFIG_STATIC_TAGS], impl.sanitizeMetricsContext(metricsContext))]);
                    }

                    if (points.length > 0) {
                        influxReporter.writePoints(metricName, points, metricsCallback(points));
                    }
                }
            }

            if (testReport.counters) {
                for (const metricName in testReport.counters) {
                    const points = [];
                    const counter = testReport.counters[metricName];

                    for (const [timestamp, count, metricsContext] of counter) {
                        points.push([{
                            time: timestamp,
                            value: count
                        }, Object.assign(instance.config[constants.CONFIG_STATIC_TAGS], impl.sanitizeMetricsContext(metricsContext))]);
    ...
    api = {
        init: function (scriptConfig, eventEmitter) {
            ...
            eventEmitter.on('stats', function (report) {
                ...
                // Transform the report to the new metrics object that we expose for graphs.
                const metrics = report.metrics();

                impl.reportResults(that, reporter, metrics);
                impl.reportErrors(that, reporter, metrics);
                impl.reportCustomMetrics(that, reporter, metrics);
            });
        }
    };
