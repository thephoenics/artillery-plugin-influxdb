            ...
            while (index < testReport.latencies.length) {
                ...
                if (JSON.parse(JSON.stringify(instance.config[constants.CONFIG_INFLUX].matches)) === constants.CONFIG_SHOW_MATCHES) {
                    points.push([{
                        ...
                        response: statusCode
                    }, instance.config[constants.CONFIG_STATIC_TAGS], metricsContext)]);
                } else {
                    points.push([{
                        time: timestamp,
                        value: latency / 1000000
                ...
                for (const metricName in testReport.histograms) {
                    ...
                    for (const [timestamp, delta, metricsContext] of histogram) {
                        points.push([{
                            time: timestamp,
                            value: delta / 1000000
                        }, Object.assign(instance.config[constants.CONFIG_STATIC_TAGS], metricsContext)]);
                    }

                    if (points.length > 0) {
                        influxReporter.writePoints(metricName, points, (err) => {
                            if (err) {
                                impl.handleError(err.message);
                            } else {
                                console.log(util.format(messages.metricsReportedToInflux, points.length));
